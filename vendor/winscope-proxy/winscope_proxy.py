...
MAX_FETCH_SIZE_BYTES = 375 * 1024 * 1024
...
LAST_WINSCOPE_SESSION_DIR = '/data/local/tmp/last_winscope_tracing_session'
...
def terminate_export_process(process, process_group: int) -> None:
    ...
    try:
        ...
        log.warning("Export process did not exit after SIGKILL")
    except (OSError, ValueError):
        pass


def _drain_export_stderr(stderr, stderr_tail: bytearray) -> None:
    """持续读取导出进程 stderr，仅保留最后 EXPORT_STDERR_LOG_LIMIT 字节。"""
    try:
        while chunk := stderr.read(64 * 1024):
            if len(chunk) >= EXPORT_STDERR_LOG_LIMIT:
                stderr_tail[:] = chunk[-EXPORT_STDERR_LOG_LIMIT:]
            else:
                stderr_tail.extend(chunk)
                if len(stderr_tail) > EXPORT_STDERR_LOG_LIMIT:
                    del stderr_tail[:-EXPORT_STDERR_LOG_LIMIT]
    except (OSError, ValueError) as ex:
        log.warning("Unable to drain export stderr: {}".format(ex))


def _finish_export_stderr_reader(stderr_reader, stderr) -> None:
    """等待 reader 读到 EOF；异常场景关闭管道以使其退出。"""
    stderr_reader.join(timeout=EXPORT_TERMINATE_TIMEOUT_S)
    if stderr_reader.is_alive():
        ...
        stderr_reader.join(timeout=EXPORT_TERMINATE_TIMEOUT_S)
    if stderr_reader.is_alive():
        log.warning("Export stderr reader did not exit")
...
def call_export(command: list[str], work_dir: str) -> None:
    ...
    try:
        ...
        stderr_reader = threading.Thread(
            target=_drain_export_stderr,
            args=(process.stderr, stderr_tail),
            name="winscope-export-stderr",
            daemon=True,
        )
        stderr_reader.start()
        try:
            process.wait(timeout=EXPORT_TIMEOUT_S)
        except subprocess.TimeoutExpired as ex:
            terminate_required = True
            raise ExportError("Export command timed out") from ex
        _finish_export_stderr_reader(stderr_reader, process.stderr)
        if process.returncode != 0:
            log.warning(
                "Export command failed: {}".format(
...
class _ExportEndpoint(DeviceRequestEndpoint):
    def _export(self, server, device_id, export_arguments):
        ...
        try:
            ...
            log.exception("Unable to export archive")
            raise ExportError("Unable to export archive") from ex
        finally:
            if work_dir:
...
class ExportSessionEndpoint(_ExportEndpoint):
    def process_with_device(self, server, path, device_id):
        if path:
            raise BadRequest("Unexpected path after device id")
        self._export(
            server,
            device_id,
            ["--session-dir", LAST_WINSCOPE_SESSION_DIR],
        )
...
class DownloadEndpoint(RequestEndpoint):
    ...
    def process(self, server, path):
        ...
        try:
            ...
            archive_filename = time.strftime(
                "winscope-session-%Y%m%d-%H%M%S.zip",
                time.localtime(os.path.getmtime(archive_path)),
            )
            server.send_response(HTTPStatus.OK)
            response_started = True
...
class ADBWinscopeProxy(BaseHTTPRequestHandler):
    def __init__(self, request, client_address, server):
        ...
        self.router.register_endpoint(
            RequestType.POST, "exportsession", ExportSessionEndpoint())
